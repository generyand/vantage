...
def upgrade() -> None:
    ...
    op.execute(
        "ALTER TABLE governance_areas DROP CONSTRAINT IF EXISTS check_governance_area_type"
    )
    ...
    op.execute(
        "UPDATE governance_areas SET area_type = CASE "
        "WHEN area_type = '1' THEN 'Core' "
        "WHEN area_type = '2' THEN 'Essential' "
        "ELSE 'Core' END"
    )

    # Step 4: Create the new ENUM types in PostgreSQL
    op.execute(
        "CREATE TYPE user_role_enum AS ENUM ('SUPERADMIN', 'MLGOO_DILG', 'AREA_ASSESSOR', 'BLGU_USER')"
    )
    ...
    op.execute(
        "ALTER TABLE governance_areas ALTER COLUMN area_type TYPE area_type_enum USING area_type::area_type_enum"
    )
...
def downgrade() -> None:
    ...
    op.execute(
        "ALTER TABLE governance_areas ALTER COLUMN area_type TYPE SMALLINT USING area_type::integer"
    )
    ...
    op.execute(
        "ALTER TABLE users ADD CONSTRAINT check_user_role CHECK (role IN (1, 2, 3))"
    )
    op.execute(
        "ALTER TABLE governance_areas ADD CONSTRAINT check_governance_area_type CHECK (area_type IN (1, 2))"
    )
