...
def get_dashboard_stats(
    ...
    completed_assessments = assessment_service.get_completed_count_by_user(current_user.id)
    in_progress_assessments = assessment_service.get_in_progress_count_by_user(current_user.id)
...
def get_progress_report(
    ...
    return {
        ...
        "improvement_trend": assessment_service.calculate_improvement_trend(assessments)
    }
...
def get_performance_analysis(
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_active_user),
    category: Optional[str] = Query(default=None, description="Filter by specific category")
) -> Any:
    """
    ...
    return {
        ...
        "recommendations": assessment_service.get_development_recommendations(current_user.id)
    }
